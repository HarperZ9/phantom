...
```powershell
Get-FileHash -Algorithm SHA256 PhantomSetup-v1.1.1.msi
```
...
**v1.1.1 is not code-signed.** There is no Authenticode signature on the
MSI yet, so:

- `signtool verify /pa PhantomSetup-v1.1.1.msi` reports **no signature**.
