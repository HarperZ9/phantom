...
The gate settles whether a drawing fits its columns and matches the spec it came
from. Both sides of that read the same JSON, so it cannot settle whether the
drawing is TRUE of the code. That is what phantom-cli/tests/repo_art.rs is for.
"""
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "phantom", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('phantom') % 100000:05d}"
